...
def _build_devin_prompt(
    ...
    upstream_service = settings.service_name
    source_repo = settings.source_repo

    prompt = f"""# URGENT: Breaking API Contract Change - Parallel Remediation Session

⚠️ **CONTEXT**: A breaking change was just deployed to {upstream_service}. This is affecting MULTIPLE downstream services simultaneously. You are one of several Devin agents working IN PARALLEL to fix different repos. Your mission: fix {service_name} while other Devin instances handle other services.
...
After your PR is created AND CI passes green, you MUST include a
`notification_bundle` inside your final `structured_output`. This bundle
drives downstream Jira ticket creation and Slack notification, so write the
content for human reviewers — not machines.

**Timing**: Do NOT emit the bundle before CI is green. The sequence is:
1. Open the PR
2. Wait for CI checks to pass
3. Confirm all checks are green
4. THEN emit `structured_output` with the `notification_bundle`
...
{{
  ...
  "assertions": {{
    ...
    "pr_url": "<the actual downstream PR url>"
  }},
  "jira": {{
    "summary": "[ACCR] {service_name}: adapt to breaking {upstream_service} contract change",
    "description_text": "<Write a clear Jira description covering: what upstream change landed in {upstream_service}, what broke in {service_name}, what this PR fixes, and which files were changed. Include the PR link and CI status.>"
  }},
  "slack": {{
    "text": "<Write a concise Slack message: what changed upstream, what this PR fixes in {service_name}, CI status, and a link to the PR for review.>",
    "blocks": []
  }}
}}
```
...
- Do NOT emit the bundle until CI is green on the PR
- Do not omit `jira.description_text`; Jira ticket creation is skipped without it
- The Jira description should include: upstream change summary, downstream impact, files changed, PR link, and CI status
- The Slack text should be actionable: tell the reviewer what to look at and link the PR
- Make it explicit that the upstream change landed in `{upstream_service}`
- Make it explicit that the PR belongs to the downstream repo `{service_info.repo}`
- State that the remediation was generated by Devin
